...
五、web_fetch：让 agent 能读线上资料
抓取公开网页并转成纯文本，用来查本地没有的文档与题面。三处实现上的取舍：
不执行 JavaScript，但保留页面内联的 JSON——洛谷题目的输入输出样例与时空
限制只存在于其中，比渲染出的文字更全；超长内容截断时保留头尾，因为有站点
把正文放在页面最末尾，只留头部会正好把要读的东西切掉；地址校验会真的做一次
DNS 解析再判断，而不是只匹配字符串——localtest.me 这类公网域名会解析到
127.0.0.1，字面量匹配挡不住，任意一条 A 记录落在内网即整体拒绝。
...
每一处设计决策的理由与被放弃的方案都记在仓库的 DESIGN.md 里，共 29 条，
